    ...
    public class Repository : IRepository
    {
        ...
        public SBUser CheckUserLoginForNormal(string userName, string password, string imi, int AppId, string EmpType)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Properties["UserId"] = "UserId=" + userName;
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                if (type == 0)
                {
                    if (user.isActive == true)
                    {
                        ...
                        {
                            logger.Properties["AppId"] = "AppId=" + AppId;
                            logger.Properties["UserId"] = "UserId=" + obj.userId;
                            logger.Error(ex.Message);
                            result.status = "error";
        ...
        private CollectionSyncResult SaveHouseCollectionSync(SBGarbageCollectionView obj, int AppId, int type)
        {
            ...
            try
            {
                ...
                using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
                {
                    ...
                    //   db.Database.Connection.EnlistTransaction(Transaction.Current);
                            ...
                            try
                            {
                                ...
                                if (result.status == "success")
                                {
                                    try
                                    {
                                        ...
                                        //  dbMain4.Database.Connection.Open();
        ...
        public CollectionSyncResult SaveGarbageCollectionOffline(SBGarbageCollectionView obj, int AppId, int type)
        {
            ...
            HouseMaster Hdata = new HouseMaster();
            dynamic house = string.Empty;
            ...
            if (obj.houseId != null && obj.houseId != "")
            {
                ...
                Hdata.houseLat = house.houseLat;
                Hdata.houseLong = house.houseLong;
                pme = "House";
                pmm = "घरा";
            }
            ...
            if (obj.LWId != null && obj.LWId != "")
            {
                ...
                var eCoord = new GeoCoordinate(Convert.ToDouble(obj.Lat), Convert.ToDouble(obj.Long));
                a = sCoord.GetDistanceTo(eCoord);
                Hdata.houseLat = house.LWLat;
                Hdata.houseLong = house.LWLong;
                pme = "Liquid Point";
                pmm = "लिक्विड पॉइंट";
            }
            ...
            if (obj.SSId != null && obj.SSId != "")
            {
                ...
                var eCoord = new GeoCoordinate(Convert.ToDouble(obj.Lat), Convert.ToDouble(obj.Long));
                a = sCoord.GetDistanceTo(eCoord);
                Hdata.houseLat = house.SSLat;
                Hdata.houseLong = house.SSLong;
                pme = "Street Point";
                pmm = "स्ट्रीट पॉइंट";
            }
            ...
            {
                if (obj.IsLocation == false && house != null && appdetails.IsScanNear == true && ((Convert.ToDouble(appdetails.Type) >= a || (Hdata.houseLat == null && Hdata.houseLong == null))))
                {
                    switch (obj.gcType)
                    {
                        case 1:
                ...
                if (obj.IsLocation == false && Convert.ToDouble(appdetails.Type) < a && appdetails.IsScanNear == true && ((Hdata.houseLat != null && Hdata.houseLong != null)))
                {
                    ...
                    result.messageMar = "तुम्ही  " + pmm + " जवळपास नाही, तुमचे  " + pmm + "पासूनचे अंतर " + a + " मीटर आहे.";
                }
            ...
            using (DevSwachhBharatNagpurEntities db = new DevSwachhBharatNagpurEntities(AppId))
            {
                ...
                try
                {
                    ...
                    foreach (var item in obj)
                    {
                        ...
                        {
                            logger.Properties["AppId"] = "AppId=" + AppId;
                            logger.Properties["UserId"] = "UserId=" + item.userId;
                            logger.Error(ex.Message);
                            result.Add(new CollectionSyncResult()
                            {
        ...
        public CollectionSyncResult SaveAssignPartner(int AppId, int UserId, AssignTeam obj)
        {
            ...
            using (var db = new DevSwachhBharatNagpurEntities(AppId))
            {
                try
                {
                    ...
                    {
                        var IdExists = db.HSTeamDetails.Where(x => x.Sid == obj.scannerId && x.Pid == obj.partnerId && x.IsActive == true).FirstOrDefault();
                        var IdExistsInActive = db.HSTeamDetails.Where(x => x.Sid == obj.scannerId && x.Pid == obj.partnerId && x.IsActive == false).FirstOrDefault();
                        if (IdExistsInActive != null)
                        {
                            IdExistsInActive.Cid = UserId;
                            IdExistsInActive.IsActive = true;
                            IdExistsInActive.UpdateDate = DateTime.Now;
                            db.SaveChanges();
                            result.status = "success";
